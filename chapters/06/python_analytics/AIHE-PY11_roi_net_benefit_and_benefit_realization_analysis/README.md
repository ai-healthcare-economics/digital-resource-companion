...
**Additional chapter alignment:** 4, 6, 7 
**Related companion resources:** AIHE-B03 
**Data classification:** Synthetic example data only

## Purpose

Calculates gross and risk-adjusted benefits, total cost, net benefit, ROI, payback period, and the effect of realization probability and attribution assumptions.
...
```bash
python python/11_roi_benefit_realization.py
```
...
`Benefits` sheet with benefit item, gross annual amount, realization probability, attributable fraction, start/end year; `Costs` sheet with one-time and recurring costs; `Parameters` sheet with horizon and discount rate.
...
Annual cash-flow table, gross and realized benefit summaries, ROI and payback metrics, cumulative net-benefit chart, and sensitivity-ready output tables.
