...
**Related companion resources:** AIHE-D02, AIHE-RG03 
**Data classification:** Synthetic example data only

## Purpose

Models heterogeneous clinicians whose adoption changes through training, peer influence, perceived usefulness, workload, and adverse incidents.
...
```bash
python python/23_agent_based_clinician_adoption.py
```
...
`Agents` sheet with agent_id, role, initial adoption, trust, digital skill, peer influence, and workload; `Parameters` sheet with periods and effect strengths.
...
Agent-level adoption history, role summaries, adoption trajectory, heterogeneity distribution, and scenario interpretation.
